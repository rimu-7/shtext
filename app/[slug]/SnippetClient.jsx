...
export default function SnippetClient({ initialData, slug }) {
  ...
  // Unlock Function (Calls the Secure API)
    ...
    try {
      const res = await fetch("/api/snippet/verify", {
        ...
        headers: { 
          ...
          "x-api-key": process.env.NEXT_PUBLIC_WEB_API_KEY // 👈 Critical
        },
        cache: "no-store",
        body: JSON.stringify({ slug, password: passwordInput.trim() }),
      });
      ...
      // Success: API returns decrypted content
  ...
  if (isLocked) {
    return (
      <div className="w-full max-w-md bg-card border-2 border-black dark:border-white shadow-[8px_8px_0_0_rgba(0,0,0,1)] dark:shadow-[8px_8px_0_0_rgba(255,255,255,1)] p-8 text-center space-y-6">
        ...
        <div>
          <h2 className="text-2xl font-black text-foreground">Password Required</h2>
    ...
    <div className="w-full max-w-3xl space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="bg-card p-4 border-2 border-black dark:border-white shadow-[4px_4px_0_0_rgba(0,0,0,1)] dark:shadow-[4px_4px_0_0_rgba(255,255,255,1)] flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="flex items-center text-foreground font-mono font-bold text-sm bg-accent/30 px-3 py-1 border-2 border-black/10 dark:border-white/10">
          ...
          <span>
             {initialData.expireAt === "Never" ? "Expires: Never" : `Expires: ${new Date(initialData.expireAt).toLocaleString()}`}
          </span>
        </div>
        <div className="flex gap-4">
          <Button onClick={copyLink} variant="outline" className="h-10 border-2 border-black dark:border-white shadow-[4px_4px_0_0_rgba(0,0,0,1)] dark:shadow-[4px_4px_0_0_rgba(255,255,255,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0_0_rgba(0,0,0,1)] hover:dark:shadow-none transition-all font-bold">
            Copy Link
          </Button>
          <Button onClick={copyText} className="h-10 border-2 border-black dark:border-white shadow-[4px_4px_0_0_rgba(0,0,0,1)] dark:shadow-[4px_4px_0_0_rgba(255,255,255,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0_0_rgba(0,0,0,1)] hover:dark:shadow-none transition-all font-bold bg-primary text-primary-foreground">
      ...
      <div className="bg-card border-2 border-black dark:border-white shadow-[8px_8px_0_0_rgba(0,0,0,1)] dark:shadow-[8px_8px_0_0_rgba(255,255,255,1)] overflow-hidden">
        ...
        <div className="bg-muted border-b-2 border-black dark:border-white px-4 py-3 flex items-center justify-between">
            ...
            <span className="text-xs font-mono font-bold uppercase tracking-wider text-muted-foreground">Decrypted Content</span>
        </div>
      ...
      <div className="text-center pt-8">
        <Link href="/" className="text-sm font-bold text-muted-foreground hover:text-primary hover:underline">
